...
let saveDetailInfoDoctor = (inputData) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!inputData.doctorId || !inputData.contentHTML || !inputData.contentMarkdown || !inputData.action || !inputData.selectedPrice || !inputData.selectedPayment || !inputData.selectedProvince) {
        resolve({
          errCode: 1,
...
let getScheduleByDate = (doctorId, dateTimestamp) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId || !dateTimestamp || isNaN(dateTimestamp)) {
        return resolve({
          errCode: 1,
          errMessage: "Missing or invalid required parameter!",
        });
      }
      ...
      if (isNaN(dateObj.getTime())) {
        return resolve({
          errCode: 2,
          errMessage: "Invalid timestamp format!",
        });
      }
        ...
        let data = await db.Doctor_Infor.findOne({
          ...
          include: [
            ...
            },
            {
              model: db.Clinic,
              as: "clinicData",
              attributes: ["name", "address"], // Lấy tên và địa chỉ phòng khám
            },
          ],
          raw: false,
          nest: true,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.email || !data.doctorId || !data.patientId || !data.timeType || !data.imgBase64) {
        resolve({
          errCode: 1,
          errMessage: "Missing required parameter12!",
        });
      } else {
        ...
        resolve({
          errCode: 0,
          errMessage: "Save remedy succeed!",
        });
      }
    } catch (e) {
      reject(e);
